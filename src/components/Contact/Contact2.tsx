  ...
  return (
    <label className="grid grid-columns-1 justify-items-center w-3/4 my-3">
      ...
      <input
        ...
        className={
          "w-3/4 h-6 text-slate-600 bg-blue-100 p-0.5 rounded-sm active: outline-none"
        }
...
function handleValidate(value: string, setErroStyle: any, name: string) {
  const error = () => {
    ...
    if (name === "name") {
      ...
      if (emailValidateRegex.test(value) === true && value.length < 35) {
        return false
      } else {
        return true
      }
    } else if (name === "number") {
      if (numberValidateRegex.test(value) === true && value.length < 16) {
        return false
      } else {
        console.log(value)
        return true
      ...
      <div className="flex justify-between content-center rounded-lg bg-white w-7/12 py-5 min-h-50 shadow-lg">
        ...
        <div className="grid grid-flow-row h-full p-5 min-h-70 w-7/12">
          ...
          <div className="flex place-content-endh10 text-lg text-slate-700">
            <p className="h-10">
              Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sedn
              dsaadda dlsdds
            </p>
          </div>
        </div>
